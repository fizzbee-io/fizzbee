...
func (h *Heap) Clone(refs map[string]*lib.Role, channelRefs map[int]*lib.Channel, permutations map[lib.SymmetricValue][]lib.SymmetricValue, alt int) *Heap {
    return &Heap{state: CloneDict(h.state, refs, channelRefs, permutations, alt), globals: h.globals}
}
...
func (s *Scope) GetAllVisibleVariables(roleRefs map[string]*lib.Role, channelRefs map[int]*lib.Channel) starlark.StringDict {
    dict := starlark.StringDict{}
    s.getAllVisibleVariablesResolveRoles(dict, roleRefs, channelRefs)
    return dict
}

func (s *Scope) getAllVisibleVariablesResolveRoles(dict starlark.StringDict, roleRefs map[string]*lib.Role, channelRefs map[int]*lib.Channel) {
    if s.parent != nil {
        s.parent.getAllVisibleVariablesResolveRoles(dict, roleRefs, channelRefs)
    }
    CopyDict(s.vars, dict, roleRefs, channelRefs, nil, 0)
}
...
func CloneDict(oldDict starlark.StringDict, refs map[string]*lib.Role, channelRefs map[int]*lib.Channel, permutations map[lib.SymmetricValue][]lib.SymmetricValue, alt int) starlark.StringDict {
    return CopyDict(oldDict, nil, refs, channelRefs, permutations, alt)
}

// CopyDict copies values `from` to `to` overriding existing values. If the `to` is nil, creates a new dict.
func CopyDict(from starlark.StringDict, to starlark.StringDict, refs map[string]*lib.Role, channelRefs map[int]*lib.Channel, permutations map[lib.SymmetricValue][]lib.SymmetricValue, alt int) starlark.StringDict {
    if to == nil {
    ...
    for k, v := range from {
        ...
        newValue, err := deepCloneStarlarkValueWithPermutations(v, refs, channelRefs, permutations, alt)
        PanicOnError(err)
        to[k] = newValue
    }
    return to
...
func (c *CallFrame) Clone(refs map[string]*lib.Role, channelRefs map[int]*lib.Channel, permutations map[lib.SymmetricValue][]lib.SymmetricValue, alt int) (*CallFrame, error) {
    obj := c.obj
    if c.obj != nil {
        cloned, err := deepCloneStarlarkValueWithPermutations(c.obj, refs, channelRefs, permutations, alt)
        if err != nil {
            return nil, err
        }
        obj = cloned.(*lib.Role)
    }
    newVars := CloneDict(c.vars, refs, channelRefs, permutations, alt)
...
func (t *Thread) Execute() ([]*Process, bool) {
    ...
    for t.Stack.Len() > 0 {
        ...
        //fmt.Println("t.Stack.Len()", t.Stack.Len(), currentFrame, currentFrame.obj)
        ...
        case *ast.Statement:
            ...
            //fmt.Println("Executing statement", msg, t.Process.Roles)
            //if len(t.Process.Roles) > 1 {
            //	fmt.Printf("frame.obj: %p, roles[0]: %p, roles[1]: %p\n", frame.obj, t.Process.Roles[0], t.Process.Roles[1])
            //}
            forks, yield = t.executeStatement()
            //fmt.Printf("forks: %v, yield: %t, process.heap.state: %+v\n", forks, yield, t.Process.Heap.state)
        case *ast.ForStmt:
...
func (t *Thread) executeStatement() ([]*Process, bool) {
    ...
    if stmt.PyStmt != nil {
        ...
        if def == nil {
            ...
            fmt.Println("CallStmt: ", stmt.CallStmt.Name, newFrame.vars, stub, receiver)
            if stmt.CallStmt.Receiver != "" {
            ...
            if stub == nil || (stub.Channel.IsSynchronous() && stub.Channel.IsOrdered()) {
                ...
                channel := t.Process.Channels[stub.Channel.Id]
                fmt.Printf("Channel: %p, %d, %s\n", channel, channel.Id, channel.Name)
                channel.AddMessage(receiver.RefStringShort(), newFrame, newFrame.Name, newFrame.vars)
                t.Process.Enable()
                return t.executeEndOfStatement()
            }
            return nil, false
...
func (t *Thread) executeEndOfBlock() bool {
    ...
    for {
        fmt.Println(frame.pc)
        //if frame.scope == nil {
        //	fmt.Println("Scope is nil")
        //	t.Process.removeCurrentThread()
        //	return true
        //}
        ...
        if frame.scope == nil {
            ...
            //fmt.Printf("protobuf: type: %T, value: %v\n", protobuf, protobuf)
            if action, ok := protobuf.(*ast.Action); ok {
                if action.Name == "Init" {
                    ...
                    variables := oldScope.GetAllVisibleVariables(roleRefs, t.Process.Channels)
...
func (t *Thread) CopyInitValuesForEphemeralFields(oldFrame *CallFrame) {
    ...
    fieldsCloned, err := deepCloneStarlarkValue(fields, nil, nil)
    if err != nil {
        panic(err)
    }
