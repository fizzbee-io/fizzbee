...
type Process struct {
    ...
    ChannelMessages map[int][]*lib.ChannelMessage `json:"channelMessages"`
...
func NewProcess(name string, files []*ast.File, parent *Process) *Process {
    ...
    p := &Process{
        ...
        Stats:       NewStats(),
        Channels:    map[int]*lib.Channel{},

        ChannelMessages: map[int][]*lib.ChannelMessage{},
    }
...
func (p *Process) Fork() *Process {
    ...
    channelRefs := make(map[int]*lib.Channel)
    for _, channel := range p.Channels {
        _, err := deepCloneStarlarkValue(channel, roleRefs, channelRefs)
        if err != nil {
            panic(err)
        }
    }
    clone.SetCustomPtrFunc(reflect.TypeOf(&lib.Role{}), roleResolveCloneFn(roleRefs, channelRefs, nil, 0))
    clone.SetCustomFunc(reflect.TypeOf(starlark.Set{}), starlarkSetResolveFn(roleRefs, channelRefs, nil, 0))
    clone.SetCustomFunc(reflect.TypeOf(starlark.Dict{}), starlarkDictResolveFn(roleRefs, channelRefs, nil, 0))
    p2 := &Process{
        Name:      p.Name,
        Heap:      p.Heap.Clone(roleRefs, channelRefs, nil, 0),
        Current:   p.Current,
        Parent:    p,
    ...
    p2.Roles = MapRoleValuesInOrder(roleRefs, p.Roles)
    //fmt.Println("Fork, channels", p.Channels)
    p2.Channels = channelRefs
    return p2
}
...
func (p *Process) CloneForAssert(permutations map[lib.SymmetricValue][]lib.SymmetricValue, alt int) *Process {
    ...
    channelRefs := make(map[int]*lib.Channel)
    for _, channel := range p.Channels {
        deepCloneStarlarkValue(channel, refs, channelRefs)
    }
    clone.SetCustomPtrFunc(reflect.TypeOf(&lib.Role{}), roleResolveCloneFn(refs, channelRefs, permutations, alt))
    clone.SetCustomFunc(reflect.TypeOf(starlark.Dict{}), starlarkDictResolveFn(refs, channelRefs, permutations, alt))
    clone.SetCustomFunc(reflect.TypeOf(starlark.Set{}), starlarkSetResolveFn(refs, channelRefs, permutations, alt))
    clone.SetCustomFunc(reflect.TypeOf(lib.SymmetricValue{}), symmetricValueResolveFn(refs, channelRefs, permutations, alt))
    p2 := &Process{
        Name:      p.Name,
        Heap:      p.Heap.Clone(refs, channelRefs, permutations, alt),
        Current:   p.Current,
        Parent:    p,
        Evaluator: p.Evaluator,
...
func (p *Process) GetAllVariables() starlark.StringDict {
    ...
    CopyDict(p.Heap.state, dict, roleRefs, p.Channels, nil, 0)
    frame := p.currentThread().currentFrame()
    if frame.obj != nil {
        self, err := deepCloneStarlarkValue(frame.obj, roleRefs, p.Channels)
        if err != nil {
            panic(err)
        }
        dict["self"] = self
    }
    CopyDict(frame.vars, dict, roleRefs, p.Channels, nil, 0)
    frame.scope.getAllVisibleVariablesResolveRoles(dict, roleRefs, p.Channels)
...
func processPreInit(init *Node, stmts []*ast.Statement) {
    ...
    globals := thread.currentFrame().scope.GetAllVisibleVariables(nil, init.Channels)
    for k, v := range globals {
        fmt.Printf("Global: %p, %s: %+v\n", v, k, v)
        if v.Type() == "Channel" {
            v.(*lib.Channel).Name = k
        }
        fmt.Printf("Global: %p, %s: %+v\n", v, k, v)
    }
    for i, channel := range init.Channels {
        fmt.Printf("Channel %p, %d: %+v\n", channel, i, channel)
    }
    globals.Freeze()
...
func (p *Processor) YieldNode(node *Node) {
    ...
    for i, channel := range node.Channels {
        fmt.Println("Processing channel", i, channel.RefStringShort())
        for j, message := range channel.Messages {
            fmt.Println("Processing message", j, message)
            newNode := node.ForkForAlternatePaths(node.Process.Fork(), fmt.Sprintf("channel-%d-message-%d", i, j))
            fmt.Println("newNode.Channels", newNode.Channels)
            msg := newNode.Channels[i].Messages[j]
            newNode.Channels[i].Messages = append(newNode.Channels[i].Messages[:j], newNode.Channels[i].Messages[j+1:]...)
            thread := newNode.Process.NewThread()
            //newNode.Process.Current = len(newNode.Process.Threads) - 1
            newNode.Inbound[0].ReqId = newNode.Process.Current
            thread.Stack.Pop()
            fmt.Println(thread.Stack.Peek())
            fmt.Println("Frame:", msg.Frame())
            frame := msg.Frame().(*CallFrame)
            //if frame.obj != nil {
            //	for _, role := range newNode.Roles {
            //		if frame.obj.RefStringShort() == role.RefStringShort() {
            //			frame.obj = role
            //			break
            //		}
            //	}
...
func (p *Processor) YieldFork(node *Node, process *Process) {
    ...
    for i, channel := range process.Channels {
        fmt.Println("Processing channel", i, channel.RefStringShort())
        for j, message := range channel.Messages {
            fmt.Println("Processing message", j, message)
        }
